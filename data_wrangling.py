...
sys.path.append('/Users/kanishk/Downloads/WESAD/cvxEDA/src')
import cvxEDA
...
class SubjectData:
    ...
    def __init__(self, main_path, subject_number):
        ...
        with open(self.name + '/' + self.name + '.pkl', 'rb') as file:
    ...
    def extract_features(self):  # only wrist
        results = \
            {
                key: stats.get_statistics(self.get_wrist_data()[key].flatten(), self.labels, key)
        ...
        if feat_names is None:
            ...
            for row in x.index:
                for col in x.columns:
                    feat_names.append('_'.join([str(row), col]))
